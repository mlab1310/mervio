...
## D-044 — Périmètre des commandes : celui des rapports Shopify
...
**Décision :** Mervio adopte le périmètre publié par Shopify. Toute commande de l'export compte dans `orders`, dans le CA et dans le panier moyen, quels que soient `Cancelled at`, `Financial Status`, `Source` ou un montant nul. Le CA Mervio est défini comme « CA net de remises **avant ajustements** » : il correspond au numérateur du panier moyen Shopify (*gross sales − discounts*), **pas** aux *net sales* Shopify, qui déduisent les annulations et retours. Le statut financier n'est pas une preuve d'encaissement.
...
**Conséquence :** aucun chiffre ne change. Le signal `cancelled_orders_counted` isole désormais les commandes annulées à montant positif et sans remboursement (cas B, C, D), seule source d'écart avec les *net sales* Shopify. Le harnais ne bloque plus sur le simple fait de compter ces commandes, mais sur `unreversed_cancelled_or_unsettled_orders_in_revenue`.
**Limites :** cas B à D et E jamais observés avec un montant positif ; aucun export ne distingue une commande gratuite commerciale d'une opération non commerciale (remplacement, échantillon) ; la date d'annulation n'est pas exploitée (illisible sur OH5). Un indicateur « après ajustements » demanderait une date et un montant d'annulation que l'export ne fournit pas de façon fiable.
...
**Limites :** part marchandise du remboursement non isolable ; série mensuelle des remboursements décalée vers la date de vente ; un export d'une autre app peut porter un autre `Refunded Amount`. Un fil communautaire non vérifiable (2025) montre un `Discount Amount` égal au remboursement : non reproduit, sans effet sur le CA net (D-041), à surveiller.

## D-046 — Remise partielle : preuve empirique indisponible, risque accepté sous garde-fou
...
**État :** `PARTIAL-DISCOUNT EVIDENCE NOT AVAILABLE`. Remise à 100 % prouvée sur OH5 (30/30) ; remise partielle fondée sur l'API Admin (`subtotalPriceSet` « after discounts ») et sur la définition Shopify du panier moyen (*gross sales − discounts*).
**Décision :** D-041 est maintenue. Le risque est accepté pour la suite parce qu'une erreur de convention serait **détectée, pas silencieuse** : `subtotal_convention_contradiction` (connecteur) et le classement du harnais (`pre_discount`, `undetermined`) reposent sur l'arithmétique du `Total`, valable pour toute remise.
...
**Limites :** remise de port incluse ou non dans `Discount Amount` : non établi (sans effet sur le CA net) ; remise supérieure à la valeur marchande : non observée ; `Lineitem discount` jamais non nul sur OH5.
