...
**Statut (24/09/2026, Mission 004.4.5, tranches E1–E3) : première fonction privilégiée livrée.** `app_redact_customer(job_id)` (révision `0014`) applique la décision telle qu'écrite : possédée par le **propriétaire du schéma**, **sans nouveau rôle**, `REVOKE ALL ... FROM PUBLIC`, `EXECUTE` au seul `mervio_worker`, `SET search_path = pg_catalog, public, pg_temp` et toutes les relations qualifiées `public.` (D-049). Elle ne s'exécute que pour un travail `redact_customer` `running`, dont le jeton d'exclusion correspond, dans l'organisation du contexte, pour un service autorisé, et pour un demandeur qui a **encore** le rang `admin` — rang relu à l'exécution, pas à la mise en file. L'objet de l'opération est lu dans `jobs.payload` ; le seul paramètre est l'identifiant du travail. **Deux précisions d'implémentation :** (1) `SECURITY DEFINER` **ne contourne pas la RLS** — toutes les tables touchées sont en RLS forcée, donc le propriétaire y est soumis et la fonction reste confinée à une organisation par PostgreSQL, pas par une condition Python ; (2) `app_current_service_id()` lit `current_user`, qui vaut le **propriétaire** à l'intérieur d'une fonction `SECURITY DEFINER` : le principal est donc résolu depuis `session_user`, comme le fait déjà `app_ensure_service_principal`, et l'autorisation de service est vérifiée pour ce principal. L'exception d'immuabilité des lignes canoniques est livrée telle que décrite : `orders.customer_ref` seule, vers un tombstone seul, par le seul propriétaire de la table, toutes les autres colonnes comparées en `jsonb` pour qu'aucune ne bouge avec elle. Le roadmap 004.4 prévoyait un rôle dédié `mervio_redactor` ; c'est la présente décision, postérieure, qui s'applique — aucun rôle n'est ajouté. **La fonction de purge d'organisation reste à livrer (004.4.6).**
...
**Statut (24/09/2026, Mission 004.4.5, tranches E1–E3) : partiellement implémentée — primitives livrées, effacement pas encore exécutable de bout en bout.** Livré : `ObjectStore.delete(key)`, **idempotente** sur les trois pilotes (une clé absente n'est pas une erreur, car l'effacement est rejouable), sans joker, sans préfixe et sans suppression récursive ; la révision `0014` avec `customer_redactions` (preuve sans PII), `raw_objects.purge_reason`, le type de travail `redact_customer` et l'opération privilégiée ; le tombstone `redacted:<uuid4>` **aléatoire et distinct par client**, jamais dérivé du HMAC, de sorte qu'une réanalyse donne bien des comptes par client identiques ; le passage `available → purging` des objets porteurs d'identité (`shopify_orders`, `stripe`) **dans la transaction de l'effacement**, les objets sans identité (`shopify_products`, `google_ads`) étant laissés intacts. **Pas encore livré :** le rejeu des effacements à l'import — « un effacement n'est pas annulé par le prochain export du marchand » (E5) — et la désignation du client par l'opérateur (E6, D-062). **Conséquence assumée de ce découpage :** `purging → purged` exigera son propre point d'entrée privilégié, à ajouter en E4 ; il n'est pas anticipé ici. Le **contrat de séquence** — qui possède quelle transition, quelles vérifications E4 devra refaire, quels champs il peut toucher, quelles transitions restent interdites, et pourquoi les octets doivent partir **avant** que la ligne ne passe `purged` — est écrit en tête de la révision `0014`, là où un implémentateur de E4 le cherchera. Point à savoir : `raw_objects` ne porte **aucun déclencheur** ; sa machine à états n'est tenue que par les contraintes `CHECK` et par l'absence de droit `UPDATE`, de sorte qu'un retour en arrière est interdit par le **privilège**, pas par le schéma. La fonction de E4 devra donc porter cette garde elle-même.
**Statut (25/09/2026, Mission 004.4.5, tranche E4) : effacement exécutable de bout en bout.** La révision `0015` ajoute `app_finalize_raw_object_purge(job_id, raw_object_id)` — deuxième et dernière fonction privilégiée de l'effacement, aux mêmes conditions D-052 que la première, réécrites séparément pour que chaque fonction reste auditable seule (la protection contre la dérive est **behaviorale** : la suite adversariale de E4 rejoue un par un les refus prouvés pour E3). Elle n'autorise que `purging → purged`, ne change que `state` et `purged_at`, et rend `false` sans écrire si la ligne est déjà `purged`. Le gestionnaire `redact_customer` enchaîne : effacement en base, puis `ObjectStore.delete`, puis finalisation, objet par objet. **L'ordre est la propriété de sécurité** — marquer `purged` avant de supprimer affirmerait une destruction qui n'a pas eu lieu. **Aucune transaction commune PostgreSQL + magasin** n'existe ni n'est simulée : un `delete` en échec laisse la ligne `purging` (illisible, non détruite, reprenable) et le travail est **reprenable** (`RetryableJobError`, jamais permanent) ; une interruption après la suppression laisse la ligne `purging` sur un objet déjà vide, qu'un rejeu redétruit sans erreur puis finalise. Le type `redact_customer` entre maintenant dans `JobType`, avec sa permission de mise en file (`admin` au moins) et son gestionnaire — **ensemble**, comme le contrat l'exigeait. Aucune commande CLI de mise en file n'est ajoutée (elle viendra avec l'opérateur, E6).
**Type de travail `redact_customer` : déclaré en base seulement, temporairement.** Le dépôt impose qu'un type présent dans `JobType` ait un gestionnaire (`tests/test_jobs_unit.py`) ; ce gestionnaire est E4. Plutôt que d'affaiblir cet invariant, le type reste côté base jusqu'à E4, qui apportera ensemble l'énumération, la permission de mise en file, le gestionnaire et l'exécution. Vérifié à l'époque : aucun chemin applicatif ne pouvait traiter un tel travail (`enqueue_job` refusait le type, `claim_next_job` ne le voyait pas, `HandlerRegistry.resolve` levait, `settings.JOB_TYPES` interdisait de configurer un worker pour lui). **Levé par E4 (25/09/2026)** : l'énumération, la permission et le gestionnaire sont arrivés ensemble, et l'invariant « tout type déclaré a un gestionnaire » est de nouveau tenu — par la présence des deux, au lieu de l'absence des deux.
**Précision (004.4.5) — `g1:` :** une référence `g1:` est *effaçable* comme une `c1:` ; elle n'est simplement pas *résoluble* depuis une identité client (D-062). La contrainte de `customer_redactions` accepte donc les deux préfixes, et refuse un e-mail comme un tombstone.
...
**Mission :** 004.4.4.
**Statut (24/09/2026, Mission 004.4.4) : implémentée, acceptée avec résiduels.** Les cinq points ci-dessus sont livrés tels qu'arbitrés : matérialisation par le worker dans un temporaire qu'il crée lui-même (Q1), vérification N1 `sha256(temporaire)` contre `raw_objects.sha256` avec `object_checksum_mismatch` permanent et N2 conservée (Q2), confinement filesystem en quatre couches sans `O_NOFOLLOW` (Q4), vocabulaire d'audit `object.uploaded`/`raw_object` par `_replace` avec restauration exacte à la descente (Q10), contrat `put`/`open` sans garantie de non-écrasement, sans `stat()` ni `delete()`. Aucun fichier de `ingestion/`, `analytics/` ni `persistence/codec.py` n'a été modifié. **Résiduels reconduits :** objets orphelins après un crash d'upload et expiration de `retain_until` (ramassage et expiration en **004.9**, cf. `RETENTION_POLICY_V0.md`) ; absence de garantie de non-écrasement, compensée par des clés `uuid4` et `UNIQUE (organization_id, object_key)` (écriture conditionnelle en **004.9**, D-055) ; TOCTOU du pilote filesystem ; écarts de comportement entre `moto` et le fournisseur réel (validation contre le fournisseur en **004.9**, D-055) ; `certifi` MPL-2.0 dans le verrou de test ⚖️. **Trois régressions** ont été trouvées par la CI et corrigées avant ratification : traversée de chemin local à l'upload (`validate_local_source` ne jugeait la forme que par `is_file()`), configuration du magasin absente des workers ad-hoc du smoke conteneur, et volume nommé `/var/lib/mervio/objects` non préparé dans l'image pour l'utilisateur 10001. Aucune n'a exigé d'assouplir un contrat : le correctif a chaque fois porté sur l'appelant ou sur l'image, jamais sur `storage/` ni sur `settings.py`.
...
**Statut (24/09/2026, Mission 004.4.5, tranches E1–E3) : inchangée, E6 non commencée.** Les primitives d'effacement (magasin, schéma, opération privilégiée) sont livrées sans introduire la moindre résolution d'identité : aucune commande, aucune lecture de stdin, aucun usage de `MERVIO_IDENTITY_MASTER_KEY` n'est ajouté. Le chemin privilégié ne reçoit qu'un `customer_ref` **déjà calculé**, et la base refuse toute autre forme — un e-mail glissé dans `jobs.payload` est rejeté par l'opération elle-même, ce qui rend la frontière de D-062 vérifiable en base et non seulement par convention.

## D-063 — Effacement, purge et import concurrents : verrou consultatif d'organisation (004.4.5 E5)
**Contexte :** l'import s'exécute en **READ COMMITTED** (le dépôt ne pose aucun niveau explicite) et consulte les effacements déjà enregistrés **dans la transaction de `write_snapshot`**, entre la dérivation de la référence client et son écriture. Une instruction lit un instantané frais, mais la transaction ne la relit pas. Un effacement commité **entre** cette consultation et le commit de l'import n'est donc pas vu : la référence d'origine est écrite dans un instantané postérieur à l'effacement. E5 (rejeu des effacements à l'import, D-056) est resté **non commité** tant que cette question n'était pas tranchée.
**Problème :** ordonner l'import et les opérations destructrices d'une même organisation, sans sur-sérialiser des locataires ni des clients sans rapport, et sans inventer de mécanisme que le dépôt n'a pas.
**Invariant exigé :** **un import et une opération destructrice (effacement client, purge de boutique, purge d'organisation) portant sur la MÊME organisation sont totalement ordonnés à la frontière de transaction.** Aucun ordre n'est privilégié : les deux ordres totaux sont corrects, et c'est précisément ce qui rend la solution sûre.
**Décision :** un **verrou consultatif de transaction, de portée ORGANISATION** :
- **import** : `pg_advisory_xact_lock_shared(clé)` — **partagé** ;
- **effacement client**, et plus tard **purge de boutique** et **purge d'organisation** : `pg_advisory_xact_lock(clé)` — **exclusif** ;
- **clé** : `hashtextextended(organization_id || ':' || portée, 0)`. Construction **ratifiée telle quelle** : elle ne doit pas être modifiée en silence.

**Mécanisme, point par point.**
1. **Comportement de l'import :** le verrou **partagé** est pris **avant** la consultation du rejeu et avant toute écriture de ligne canonique, dans la transaction de `write_snapshot`.
2. **Comportement de l'effacement :** le verrou **exclusif** est pris avant que la transaction d'effacement n'atteigne sa section critique destructrice. **Précision d'implémentation (vérifiée) :** l'effacement de E4 s'étend sur **trois** transactions (`redact_customer`, `purging_objects`, puis une par objet dans `finalize_object_purge`) ; **seule la première touche des lignes canoniques**. C'est donc elle, et elle seule, qui doit porter le verrou. Les finalisations ne modifient que `raw_objects.state` : un import concurrent qui y toucherait échoue déjà proprement (`object_unavailable`), ce qui n'est pas une résurrection.
3. **Purges futures :** 004.4.6 **doit** réutiliser cette primitive au lieu d'en inventer une autre.
4. **Durée de vie :** transactionnelle. Le verrou est libéré **automatiquement** au `COMMIT` comme au `ROLLBACK`, et à la mort de la connexion. Aucune libération explicite, aucun nettoyage, aucune fuite après un arrêt brutal.
5. **Portée :** organisation. **Pas** par client.
6. **Concurrence :** partagé/partagé — plusieurs imports d'une même organisation coexistent. Exclusif/partagé — une opération destructrice et un import s'excluent mutuellement.
7. **Interblocages :** un **seul** verrou par organisation, donc aucun ordre d'acquisition à maintenir pour ce mécanisme, et l'interblocage démontré en portée client (D6) ne peut pas exister.
8. **Correction :** une collision de clé sur 64 bits ne peut causer qu'une **attente inutile**, jamais un ordonnancement incorrect : le verrou n'accorde aucun accès aux données, il ne fait qu'ordonner.

**Compatibilité vérifiée avant ratification.**
- **PostgreSQL 17.11**, épinglé par empreinte en CI et dans compose. `pg_advisory_xact_lock*` existe depuis 8.2, `hashtextextended` depuis 11.
- **Privilèges : aucun à accorder.** `EXECUTE` est déjà acquis à `PUBLIC` : vérifié `true` pour `mervio_app` et `mervio_worker` sur les trois fonctions. **Aucun rôle, aucune table, aucun `SECURITY DEFINER`, aucune table de verrous persistante.**
- **RLS : inchangée.** Un verrou consultatif est un espace de noms d'entiers, sans rapport avec les politiques. Une clé forgée depuis l'identifiant d'une autre organisation ne donnerait accès à **rien** : elle produirait une attente inutile, pas une fuite.
- **Modèle de connexion — piège ratifié :** `Database` ouvre ses connexions en **`autocommit=True`**. Une instruction exécutée **hors** d'un bloc transactionnel est donc sa propre transaction, et un verrou de transaction y est **relâché immédiatement** (mesuré : 0 verrou détenu après l'instruction). Le verrou **doit** être pris **à l'intérieur** de `Database.transaction()` / `TenantSession.transaction()`, qui ouvre un vrai bloc (mesuré : 1 verrou détenu pendant, 0 après la sortie). Pris ailleurs, la primitive est un **no-op silencieux** — c'est le mode de défaillance à surveiller en revue.
- **`TenantSession` :** le verrou se prend après l'établissement du contexte de tenant, avec `session.organization_id`, jamais avec une valeur venue d'un appelant.
- **Connexions worker, rôles de service :** inchangés. Une instance de `Database` détient au plus une connexion ; aucun pool n'existe encore (004.3), donc aucun verrou ne peut survivre à une transaction sur une connexion recyclée.

**Preuves (campagne D-063, 15 expériences sur le schéma et les rôles réels).** Une **première campagne a été écartée comme invalide** : elle s'exécutait sous `mervio_admin`, qui est `SUPERUSER` + `BYPASSRLS`, donc la RLS ne s'appliquait pas et les mesures ne décrivaient pas Mervio. Résultats retenus, sur les rôles non privilégiés que `Database` accepte réellement :
- **A — READ COMMITTED reproduit la résurrection** : la référence d'origine est écrite.
- **B — la re-vérification seule laisse une fenêtre** : un effacement commité **avant** la re-vérification fait avorter l'import (reprise, correct) ; commité **après**, la résurrection survient quand même.
- **C — SERIALIZABLE ferme la course mais sur-sérialise** : effacer un client fait échouer un import ne contenant **qu'un autre client** (C2). Cause mesurée : `orders` n'a **aucun index sur `customer_ref`** (révision 0011), donc le prédicat de l'`UPDATE` porte sur une plage préfixée par `organization_id`. C'est de plus **l'effacement** qui est annulé, pas l'import.
- **D — le verrou d'organisation ferme la course dans les DEUX ordres** : import d'abord, l'effacement rattrape ensuite les lignes neuves ; effacement d'abord, la consultation de l'import le voit. Aucun blocage entre clients sans rapport, aucun entre organisations.
- **Interblocage** : réel en portée client sans ordre total (l'un des deux imports est annulé par PostgreSQL), inexistant avec un verrou unique.
**Performance mesurée :** **1500 verrous par client = 95,5 ms** ; **un seul verrou d'organisation = 0,10 ms** ; consultation du rejeu = 2,4 ms pour 1500 clients. Aucune saturation de la table de verrous n'a été reproduite (6 imports concurrents × 1500 verrous, soit 9000 entrées, sans échec) : l'écart est de **latence**, pas un mur.
**Justification :** seule option qui rende l'ordre **total** et dont les **deux** ordres soient corrects, sans sur-sérialiser ni entre organisations ni entre imports, et dont une **primitive unique** couvre les trois opérations destructrices — ce que la portée client ne peut pas faire, une purge devant exclure *tous* les clients.
**Rejetées :** **A** (résurrection démontrée) ; **B seule** (fenêtre résiduelle démontrée ; reste utile en complément, à trancher séparément) ; **C** (ferme la course mais sur-sérialise à l'échelle de l'organisation et déplace l'échec sur l'effacement) ; **verrou par client** (95,5 ms par import et interblocage réel sans ordre total) ; **table de verrous persistante** (aucun besoin : la durée de vie transactionnelle suffit, et une table serait à purger).
**Conséquences :** E5 devient implémentable et commitable ; il prend le verrou partagé dans `write_snapshot`, et `redact_customer` le verrou exclusif. Une opération destructrice **attend** les imports en vol de son organisation — borné par la durée d'un import, acceptable pour une opération dont le délai réglementaire se compte en jours.
**Limitation explicite :** `purge_store` et `purge_organization` **n'existent pas** dans le dépôt — ni code, ni colonnes `status`/`purged_at`, ni déclencheur de clôture de D-051 ; le `JobType.PURGE` actuel est une purge de **rétention**, sans rapport. La présente décision **ne les corrige donc pas** : elle **impose** à 004.4.6 d'adopter cette primitive au moment de les implémenter, en plus de la clôture prévue par D-051.
**Risques résiduels :** un import anormalement long retarde une opération destructrice ⚖️ ; une collision de clé sur 64 bits provoque une attente inutile, jamais une incorrection ; **tout futur chemin d'écriture de lignes canoniques qui ne prendrait pas le verrou rouvrirait la fenêtre** — à vérifier à chaque ajout ; le verrou pris hors d'un bloc transactionnel serait un no-op silencieux (voir le piège ci-dessus).
**Mission :** arbitrage 004.4.5 (E5) ; implémentation E5 ; réutilisation obligatoire en 004.4.6.
**Analyse détaillée et protocole expérimental :** `docs/MERVIO_004_4_5_E5_RACE.md`.
